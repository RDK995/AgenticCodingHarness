...
| `reviewer` | **Derived — never below the work** | The independent milestone gate. Nothing verifies it except a human. It runs at no less than the highest tier that produced the current substantive diff, with `sonnet` as the floor. A reviewer weaker than the work it judges can emit a confident `PASS` and open the gate on nothing. |
