...
| `roast-requirements`, `architect`, `scope-mvp` | **High** | Their entire value is asking the question nobody had considered — the capability weaker models lack most. `scope-mvp` sits with them because deciding what a project can be proven without is the same judgement applied to scope. |
